...
class SocksServer():
    ...
    def __init__(self, socket, event=threading.Event(), bufferSize=Defaults['bufferSize']):
        ...
        if self.debug > 0:
            print "[S]", time.asctime(), "Server Starts - %s:%s" % (self.server.getsockname()[0], self.server.getsockname()[1])
    ...
    def parse_socks(self, data):  # Parses the Socks4a Headder
        ...
        # print " Data: \\x" + ('\\x'.join(x.encode('hex') for x in
        # data)),"fmt",fmt
        try:
    ...
    def establishConnection(self, s, data, sockets, SocketDict, inSrcPort):
        ...
        try:
            ...
            try:
                ...
                s.send(struct.pack('!HH', inSrcPort, len(granted)) +
                       granted)  # Send connection established responce
            finally:
                if self.debug > 4:
            ...
            try:
                ...
                s.send(struct.pack('!HH', inSrcPort, len(rejected)) + rejected)
            finally:
                if self.debug > 4:
            ...
            for s in inputready:
                try:
                    ...
                    if s == wrapper_channel:  # handle the input - main - socket
                        ...
                        # Tunna Head: First 4 bytes=incoming port and size of
                        # packet
                        head = self.sockReceive(s, 4)
                        (inSrcPort, size) = struct.unpack('!HH', head)
                        if debug > 3:
                            print "< L Received: ", "inSrcPort: ", inSrcPort, "size: ", size, "\n\t", struct.unpack('!HH', head)
                        ...
                        if size > 0:
                            ...
                            if outSock:
                                ...
                                if debug > 3:
                                    print "[E] Received empty & socket not in list: ", inSrcPort
                    else:  # Other sockets (outSockets)
                        data = s.recv(self.bufferSize)
                        if debug > 3:
                            print "> R Received: Data (client -", self.srcPort(s), ") :", len(data)

                        if data:

                            if debug > 3:
                                print "\t sending to:", SocketDict[self.srcPort(s)][0], "len", len(data)
                            if debug > 4:
                        ...
                        if len(data) == 0:
                            if debug > 3:
                                print "\tClosing port: ", SocketDict[self.srcPort(s)][0], 'len:', len(data), "Local Port:", self.srcPort(s)

                            if debug > 4:
                                print "[T] Write to channel locking 2"
                            self.lock.acquire()
                            try:
                                # send empty to lSrc will close the socket on
                                # the other end
                                wrapper_channel.send(
...
if __name__ == '__main__':
    ...
    if sys.argv < 1:
        ...
        try:
            ...
            if DEBUG > 0:
                print "[S] Shutting Down SocksServer"
            sys.exit(9)
